...
class Level(commands.Cog):
    ...
    async def initialize(self, bot):
        ...
        self.bot.db = await aiosqlite.connect("expData.db")
    ...
    async def level(self, ctx, member: discord.Member = None):
        ...
        async with self.bot.db.execute(
            ...
            # calculate rank
        async with self.bot.db.execute(
            "SELECT exp FROM guildData WHERE guild_id = ?", (ctx.guild.id,)
        ) as cursor:
            rank = 1
    ...
    async def leaderboard(self, ctx):
        ...
        embed = discord.Embed(
            title=f"Leaderboard Page {current}",
            description="",
...
def setup(bot):
    bot.loop.create_task(Level.initialize(self=Level(bot), bot=bot))
